...
func_chi_sq = @(x) deal(sum((x - mean(x)).^2 / mean(x)), ...
    1 - chi2cdf(sum((x - mean(x)).^2 / mean(x)), numel(x) - 1) );

fprintf('\n[proportion]\n')
data = cellfun(@(x) x(~isnan(x)), data, uni=0);
for side_i = 1:(length(data)/3)

    [~,idx] = max(cat(1,data{ [1:3] + (side_i-1)*3 }));
    counts = [sum(idx==1), sum(idx==2), sum(idx==3)];
    n = sum(counts);
    expected = n * [1/3 1/3 1/3];
    
    disp(counts);

    chi_sq = sum((counts - expected).^2 ./ expected);
    df = 2;
    p = 1 - chi2cdf(chi_sq, 2);
    fprintf('χ²(%d) = %.2f, p = %g\n', df, chi_sq, p);

    p = 1 - binocdf(counts(1)-1, sum(counts), 1/3);
    fprintf('binomial p = %g\n\n',p);
    p = 1 - binocdf(counts(2)-1, sum(counts), 1/3);
    fprintf('binomial p = %g\n\n',p);
    p = 1 - binocdf(counts(3)-1, sum(counts), 1/3);
    fprintf('binomial p = %g\n\n',p);
end
